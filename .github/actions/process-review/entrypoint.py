...
def main():
    ...
    global pref_list
    pref_list = ['Change','Question','Concern','Discussion','Praise','Suggestion','Nitpick','Guide','⚠️ ']
    # setup arguments
    args = setup_args()
    github = args.token
    global pr
    pr = args.pr_number
    print("Running in job %s on %s with pull number  %s" % (job_name, repository_name, args.pr_number))
